...
const useChangeWorklogForm = (refresh: () => void) => {
    const month = ref('');
    ...
    const worklogId = computed(() => route.params?.worklogId);

    const changeWorklog = async () => {
        if (
            worklogId.value === undefined ||
        ...
        try {
            const addedWorklog = await $fetch<MonthlyWorkLog>(`/api/worklogs/${worklogId.value}`, {
                method: 'PATCH',
                body: {
            ...
            formResponse.value = {
                isSucceed: true,
                message: `Successfully patched worklog with id ${addedWorklog.id} of employee ${addedWorklog.employeeId}`,
            };
...
export default useChangeWorklogForm;
